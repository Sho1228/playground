            ...
            try:
                ...
                try:
                    ...
                    if len(numbers_found) >= 3 and 'follower_count' not in profile_data:
                        ...
                        ColorPrint.print_success("Position-based assignment:")
                ...
                # Bio extraction - fixed using JSON data first
                try:
                    bio_match = re.search(r'"biography":"(.*?)"', page_source)
                    if bio_match:
                        bio_raw = bio_match.group(1)
                        biography = html.unescape(bytes(bio_raw, 'utf-8').decode('unicode_escape'))
                        profile_data['bio'] = biography
                        ColorPrint.print_success(f"Found bio from JSON: {biography[:50]}...")
                    else:
                        bio_selectors = [
